...
<body>
	...
	<div id="content" class="content-wrapper p-3">
  	...
  	<h2 class="display-4 text-dark">Rentals</h2>
				...
				<div class="box-body">
					<table id="myTable" class="table table-bordered display nowrap w-100">
						...
						<tbody>
							<?php
								$cnt = 1;
								$fetchRents = $functions->fetchRents();
								while($row = mysqli_fetch_array($fetchRents)) {
							?>
							<tr>
								<td><?= $cnt; ?></td>
								<td><?= $row['fullname']; ?></td>
								...
								<td>
									<?php
										if ($row['status'] == 'Pending') { ?>
											<a href="accept-rent.php?rent_id=<?= $row["rent_id"]?>" class="btn btn-info w-100">Accept</a>
									<?php } else {
